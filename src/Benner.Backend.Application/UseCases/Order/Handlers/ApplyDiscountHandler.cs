...
{
    public class ApplyDiscountHandler : ICommandHandler<ApplyDiscountCommand, Result<Domain.Entities.Order>>
    {
        private readonly IOrderRepository _orderRepository;

        public ApplyDiscountHandler(IOrderRepository orderRepository)
        {
            _orderRepository = orderRepository ?? throw new ArgumentNullException(nameof(orderRepository));
        }

        public async Task<Result<Domain.Entities.Order>> HandleAsync(ApplyDiscountCommand command, CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                order.ApplyDiscount(command.DiscountAmount);
            ...
            {
                return Result<Domain.Entities.Order>.Failure($"Erro ao aplicar desconto: {ex.Message}");
            }
        }
    }
}