...
{
    public class UpdateProductHandler : ICommandHandler<UpdateProductCommand, Result<Domain.Entities.Product>>
    {
        ...
        public async Task<Result<Domain.Entities.Product>> HandleAsync(UpdateProductCommand command, CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                product.UpdateInfo(command.Name, command.Description, command.Price, command.Category, command.Brand);
            ...
            {
                return Result<Domain.Entities.Product>.Failure($"Erro ao atualizar produto: {ex.Message}");
            }
        }
    }
}